...
get '/allow2' do
	...
	open(params[:state]) do |file|
		RestClient.post 'http://api.mxit.com/user/media/file/' + 'MemeForge' + '?fileName=' + params[:state][37..params[:state].length], file, :authorization => 'Bearer ' + JSON.load(response)['access_token']
    end
